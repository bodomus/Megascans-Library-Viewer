...
public static class DiagnosticsFormatter
{
    ...
    public static IReadOnlyList<DiagnosticField> CreateFields(DiagnosticsSnapshot snapshot)
    {
        ...
        [
            ...
            new("UE readiness rule version", snapshot.UnrealReadinessRuleVersion.ToString(CultureInfo.InvariantCulture)),
            new("Last UE readiness evaluation", FormatTimestamp(snapshot.LastUnrealReadinessEvaluation)),
            new("UE ready asset count", snapshot.UnrealReadyCount.ToString(CultureInfo.InvariantCulture)),
            new("UE ready with warnings count", snapshot.UnrealReadyWithWarningsCount.ToString(CultureInfo.InvariantCulture)),
            new("UE not ready asset count", snapshot.UnrealNotReadyCount.ToString(CultureInfo.InvariantCulture)),
            new("UE unknown asset count", snapshot.UnrealUnknownCount.ToString(CultureInfo.InvariantCulture)),
            new("UE not applicable asset count", snapshot.UnrealNotApplicableCount.ToString(CultureInfo.InvariantCulture)),
            new("UE readiness recalculation required", snapshot.RequiresUnrealReadinessRecalculation ? "Yes" : "No"),
